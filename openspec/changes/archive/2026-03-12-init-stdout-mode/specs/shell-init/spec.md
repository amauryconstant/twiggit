...
**Reason**: Stdout output is now the default, making --dry-run redundant.

**Migration**: Use `twiggit init [shell]` without --install flag instead of `twiggit init --dry-run`.

---

### Requirement: Shell Flag

**Reason**: Shell type is now specified as positional argument.

**Migration**: Use `twiggit init zsh` instead of `twiggit init --shell=zsh`.

---

### Requirement: Check Flag

**Reason**: With eval-based activation, there is no persistent "installed" state to check.

**Migration**: Use shell built-in `type twiggit` to check if function is defined in current shell session.

...
**Input**: The argument after `/osc-explore` is whatever the user wants to think about. Could be:
...
Capture the artifact(s) the user requested without asking them to invoke another workflow command. If they asked only to start a change, stop after scaffolding and show its status. When the requested capture is done, stop there and name where the work continues: `/osc-propose` writes the remaining planning artifacts, and `/osc-apply` implements the change once tasks exist. Capturing artifacts never starts implementing them.
...
- **Flow into a proposal**: "Ready to start? Run `/osc-propose` and this becomes a change."

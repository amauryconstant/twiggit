...
**Input**: Optionally specify a change name after `/osc-continue` (e.g., `/osc-continue add-auth`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
1. **Select the change**
   ...
   Always announce: "Using change: <name>" and how to override (e.g., `/osc-continue <other>`).
...
3. **Act based on status**:
   ...
   **If all planning artifacts are complete (`isPlanningComplete: true`, or legacy `isComplete: true`)**:
   ...
   - Suggest: "Planning is complete! You can now implement this change with `/osc-apply`. Once implementation and any tracked work are complete, archive it with `/osc-archive`."
   - STOP
...
- Prompt: "Run `/osc-continue` to create the next artifact"

...
**Input**: Optionally specify a change name after `/osc-sync` (e.g., `/osc-sync add-auth`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
1. **Select the change**
   ...
   Always announce: "Using change: <name>" and how to override (e.g., `/osc-sync <other>`).
